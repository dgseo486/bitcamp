...
CREATE TABLE APPL (
	...
	SNO      INTEGER      NULL     COMMENT '수강신청자번호' -- 수강신청자번호
)
COMMENT '수강신청';
...
CREATE TABLE TCHR (
	...
	COL INTEGER NULL     COMMENT '시강료' -- 시강료
)
COMMENT '강사';
...
CREATE TABLE MGR (
	...
	COL4 VARCHAR(255) NULL     COMMENT '사진' -- 사진
)
COMMENT '담당자';
...
-- 교실
ALTER TABLE CLS_RM
	ADD CONSTRAINT PK_CLS_RM -- 교실 기본키
		PRIMARY KEY (
			CRNO -- 교실번호
		);
...
CREATE TABLE LECT (
	...
	id    INTEGER      NULL     COMMENT '담당자번호' -- 담당자번호
)
COMMENT '강좌';
...
CREATE TABLE CLS_PHOT (
	CPNO <데이터 타입 없음> NOT NULL COMMENT '교실사진번호', -- 교실사진번호
	PATH <데이터 타입 없음> NOT NULL COMMENT '사진', -- 사진
	CRNO INTEGER            NOT NULL COMMENT '교실번호' -- 교실번호
)
COMMENT '교실사진';
...
ALTER TABLE CLS_PHOT
	MODIFY COLUMN CPNO <데이터 타입 없음> NOT NULL AUTO_INCREMENT COMMENT '교실사진번호';
...
-- 강좌
ALTER TABLE LECT
	ADD CONSTRAINT FK_CLS_RM_TO_LECT -- 교실 -> 강좌
		FOREIGN KEY (
			CRNO -- 교실번호
		)
		REFERENCES CLS_RM ( -- 교실
			CRNO -- 교실번호
		);
...
-- 교실사진
ALTER TABLE CLS_PHOT
	ADD CONSTRAINT FK_CLS_RM_TO_CLS_PHOT -- 교실 -> 교실사진
		FOREIGN KEY (
			CRNO -- 교실번호
		)
		REFERENCES CLS_RM ( -- 교실
